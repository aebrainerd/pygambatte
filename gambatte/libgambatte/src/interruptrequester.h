...
public:
	...
	void print_timing_state(void) {
		for (int i=intevent_unhalt; i != intevent_interrupts; i++) {
			IntEventId id_ = static_cast<IntEventId>(i);
			std::cout << i << " : "  << this->eventTime(id_) << std::endl;
		}
	}
